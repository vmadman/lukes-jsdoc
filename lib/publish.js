...
function updateItemName ( item ) {
	...
	if( attributes && attributes.length ) {
		itemName = util.format(
			'%s<span class="signature-attributes">%s</span>', itemName, attributes.join( ', ' )
		);
	}
			...
			function( item ) {
				...
				if( !hasOwnProp.call( item, 'longname' ) ) {
					...
					if( methods.length ) {
						...
						methods.forEach(
							function( method ) {
								var name = isRpc ? (typeof method.rpc === 'string') ? method.rpc : method.name : method.name;
								itemsNav += "<li data-type='method'>";
...
function buildNav ( members ) {
	...
	members.api = find( {kind : 'function'} ).filter(
		function( fnDoclet ) {
	...
	members.rpc = members.classes.filter(
		function( doclet ) {
			return doclet.rpc;
		}
	).map(
